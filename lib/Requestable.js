...
class Requestable {
    ...
    * @param {Number} [maxPages] - the maximum number of pages to return from the search
    * @param {Number} [_current] = the current page count (for internal use only)
    * @return {Promise} - a promise which will resolve when all pages have been fetched
    * @deprecated This will be folded into {@link Requestable#_request} in the 2.0 release.
    */
   _requestAllPages(path, options, cb, results, maxPages, _current) {
      results = results || [];
      ...
      return this._request('GET', path, options)
         .then((response) => {
            ...
            if (nextUrl && !(maxPages && _current >= maxPages)) {
               log(`getting next page: ${nextUrl}`);
               return this._requestAllPages(nextUrl, options, cb, results, maxPages, _current+1);
            }
